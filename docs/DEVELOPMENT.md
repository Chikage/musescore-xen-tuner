...
- Update `../tools/tuning-config-web/types.js` if there are any changes to the data structures of objects.

- Runtime logic is split by responsibility under `Xen Tuner/runtime/modules/`; `Xen Tuner/runtime/fns.js` is the compatibility loader for QML and the browser tuning-config generator.

## Project layout

- `Xen Tuner/`: MuseScore-discoverable QML plugin entry points.
- `Xen Tuner/runtime/`: shared JavaScript runtime, lookup tables, and responsibility-split modules.
- `tunings/`: bundled tuning configuration library and the default runtime lookup folder for tuning files.
- `tools/dev/`: maintainer scripts and parser tests that should be run from the project root.
- `tools/tuning-config-web/`: browser-based tuning config generator.
- `tools/midi/`: MIDI/MPE conversion helpers.
- `docs/`: long-form documentation and screenshots.
- `examples/`: sample generated data and reference files.
- `logs/`: runtime debug logs written by the plugin when debug logging is enabled.
...
![](images/minimum-distance-set-style-default.png)
...
A#\    ,  92.18c,  0
G#x\\  ,  94.13c,  -1
... etc (see ../examples/2.3.5 JI tuning table example.csv for all 315 notes)
```
...
> 
> Refer to `../tools/tuning-config-web/types.js` instead.
...
There's JSDoc documentation for the functions in the modules under `Xen Tuner/runtime/modules/`.
