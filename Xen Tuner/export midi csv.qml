...
MuseScore {
      ...
      onRun: {
        ...
        for (var staff = startStaff; staff <= endStaff; staff++) {
          ...
          for (var voice = 0; voice < 4; voice++) {
            ...
            while (true) {
              ...
              if (cursor.segment) {
                if (voice === 0) {
                  Fns.appendNativeKeySigConfigEvent(configs, cursor, nativeKeySigState);
                }
          ...
          parms.staffConfigs[staff] = configs.sort(Fns.sortConfigUpdateEvents);
        }
        ...
        for (var staff = startStaff; staff <= endStaff; staff++) {
          for (var voice = 0; voice < 4; voice++) {
            ...
            while (cursor.segment && (fullScore || cursor.tick < endTick)) {
              ...
              nextConfigIdx = Fns.applyConfigsUpTo(staffConfigs, parms, cursor.tick, nextConfigIdx);
        ...
        if (success) {
          ...
          messageDialog.text = 'exported to ' + exportPath + '.\nRun "python3 tools/midi/generate-mpe.py ' + exportPath + '" to generate MPE MIDI files.';
        } else {
